...
public class EntityTestUtil {
    ...
    public static InstancioApi<Visit> visit() {
        return Instancio.of(Visit.class)
            .generate(field(Visit::getStartTime), gen -> gen.temporal().instant().truncatedTo(ChronoUnit.MILLIS))
                .generate(field(Visit::getDuration), gen -> gen.ints().min(1));
    }

    public static InstancioApi<Visit> validVisit() {
        return visit()
                .generate(field(Visit::getDuration), gen -> gen.ints().min(Visit.MIN_DURATION));
    }

    // Generated by Claude
    public static InstancioApi<OngoingVisit> ongoingVisit() {
        return Instancio.of(OngoingVisit.class)
            .generate(field(OngoingVisit::getStartTime), gen -> gen.temporal().instant()
            .truncatedTo(ChronoUnit.MILLIS));
    }

    public static InstancioApi<OngoingVisit> validOngoingVisit() {
        return ongoingVisit()
            .generate(field(OngoingVisit::getStudentId), gen -> gen.ints().min(1));
    }
